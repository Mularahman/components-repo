    ...
    return (
        <PaginationItem key={`#{page}`}>
            <PaginationLink
                ...
                isActive={current === page} size={undefined}>
                {page}
            </PaginationLink>
        </PaginationItem>
    );
...
const ItemLinks = ({ current, total, onChange }: TablePaginationProps) => {
    ...
    for (let page = startPage; page <= endPage; page++) {
        itemLinks.push(
            <ItemLink key={page} current={current} page={page} onChange={onChange} />
        );
    }
    ...
    itemLinks.unshift(
        <ItemLink key={1} current={current} page={1} onChange={onChange} />
    );
